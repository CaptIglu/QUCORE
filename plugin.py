...
from .map_tools import WaypointMapTool


def _log_silent_exception(e, context=""):
    """Centralized silent exception logging for QUCORE plugin."""
...
def get_major_version():
    ...
    try:
        ...
        _log_silent_exception(e, "plugin.py")
    return "1"
...
class DroneCorridorPlanner(object):
    ...
    def load_config_params(self):
        ...
        # Check QSettings for license activation
        from qgis.PyQt.QtCore import QSettings
        settings = QSettings()
        saved_key = str(settings.value("QUCORE/license_key", ""))
        
        defaults["commercial_unlocked"] = verify_license_key(saved_key)
    ...
    def open_altitude_table(self):
        ...
        self.altitude_table_dialog.sigToggleWaypointLabels.connect(self.on_toggle_waypoint_labels)
        self.altitude_table_dialog.sigWaypointFocused.connect(self.on_waypoint_focused)
        self.altitude_table_dialog.sigClearFocus.connect(self.on_clear_focus)
    ...
    def on_toggle_waypoint_labels(self, checked):
        if not hasattr(self, 'lyr_waypoints') or not self.lyr_waypoints:
            return
        if checked:
            ...
            self.lyr_waypoints.setLabeling(labeling)
            self.lyr_waypoints.setLabelsEnabled(True)
        else:
            self.lyr_waypoints.setLabelsEnabled(False)
        self.lyr_waypoints.triggerRepaint()
        if self.canvas:
            self.canvas.refresh()

    def on_waypoint_focused(self, row):
        self.on_clear_focus()
